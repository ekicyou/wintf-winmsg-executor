...
## 8. 設計合成（Design Synthesis）

設計フェーズで 3 レンズを適用した結果:

- **Generalization**: 新機能の一般化は不要（移行）。横断的な移行規約——newtype の `.0` アンラップ、NULL 引数の使い分け（`Option<HWND>::None` 系 vs 生 `HWND::default()` 系）、`Result` 畳み込み、`w!` 文字列マクロ、`GetMessageW` の `BOOL` 三値判定温存——を「一般化された規約」として全コンポーネントに一律適用する。これにより実装者間の解釈ドリフトを防ぐ。
- **Build vs Adopt**: `windows` 0.62 を **adopt**（Microsoft 公式・active・MIT/Apache-2.0、steering の技術方針と整合）。`HHOOK` の `windows_core::Free`（自動 Unhook）は **adopt しない**——現行の手動 Drop が `UnhookWindowsHookEx` に加えて thread-local ポインタ（`MSG_FILTER_HOOK`）解放と `Box` 回収を兼ねるため、自動 Free では後始末が欠落する。`Send` ラッパーは **build しない**（`spawn_unchecked` ゆえ不要見込み、Phase 0 で検証）。
- **Simplification**: 新コンポーネント・新ファイルをゼロに保つ。投機的な `Send` ラッパー・抽象レイヤを導入せず、既存 3 ファイル + `Cargo.toml` の in-place 移行という最小構成とする。Send ラッパーは Phase 0 検証で必要が実証された場合に限り追加する。
