...
## Introduction
この機能は、`wintf-winmsg-executor` クレートの Win32 バインディングを `windows-sys`（0.61 系）から `windows`（0.62 系）へ置き換える、挙動非変更の内部移行である。本クレートの保守担当者を主な利用者とし、型安全性の向上とエラーハンドリングの慣用化により将来の保守性を高めることを目的とする。移行後も `cargo build` / `cargo test` / `cargo doc` がすべて通り、メッセージループ・wake・モーダル対応・パニック持ち回りといった観測可能な振る舞いが従来と同一であることを成功条件とする。生 Win32 API を直接呼ぶ既存スタイルと A 系（ANSI）API の使用は維持し、高水準ラッパへの全面書き換えや W 系 API への切り替えは行わない。

## Boundary Context
- **In scope**:
  - `Cargo.toml` の依存を `windows-sys` から `windows = "0.62"` へ差し替え、使用モジュール単位で features を再定義し、未使用 feature（`Win32_Graphics_Gdi` 候補）を棚卸しする。
...
- **Out of scope**:
  ...
  - A 系（ANSI）API から W 系（Unicode）API への切り替え。
...
3. Where ある Win32 feature モジュール（`Win32_Graphics_Gdi` を含む）が移行後のソースコードから一切参照されない場合, the 移行後のクレート shall その feature を依存宣言から除外する。
...
#### Acceptance Criteria
1. The 移行後のソースコード shall 生 Win32 API を直接呼び出す基本スタイル（A 系 ANSI API の使用を含む）を維持する。
...
4. The 移行後のソースコード shall 固定の文字列リテラルを `windows` の文字列マクロ（`s!` 等）を用いて表現する。
