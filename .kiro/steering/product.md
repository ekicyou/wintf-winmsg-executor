...
- **基本記録 (base reference)**: 上流の公開 API・設計知見は [docs.rs v0.3.2](https://docs.rs/winmsg-executor/0.3.2/winmsg_executor/) を正典とする。現行コードの公開 API（`spawn_local` / `block_on` / `MessageLoop` / `JoinHandle` / `FilterResult` / `util`）はこの正典と一致している。
- **バージョン採番**: フォーク時に SemVer を `0.0.x` 系へ振り直しており（現行 `Cargo.toml` version `0.0.3`）、上流の `0.3.2` とは独立に採番する。crate 名も `wintf-winmsg-executor` にリブランド済み。
...
_updated_at: 2026-06-19 — リブランド（wintf-winmsg-executor）と SemVer 振り直し（0.0.3）を反映_
